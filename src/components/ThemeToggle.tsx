...
const ThemeToggle = () => {
  ...
  return (
    <div className="fixed right-4 top-1/2 translate-y-8 z-50">
      <div className="bg-white/90 backdrop-blur-sm border border-gray-200 rounded-full p-2 shadow-lg hover:shadow-xl transition-all duration-300">
        ...
        >
          {theme === 'dark' ? (
            <Sun size={24} className="text-gray-600 hover:text-school-red transition-colors" />
          ) : (
            <Moon size={24} className="text-gray-600 hover:text-school-red transition-colors" />
          )}
